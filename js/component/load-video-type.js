...
<div class="row" id="div-video-wrapper" where="" uid="">
...
export default class VidepType extends HTMLElement {
    ...
    attributeChangedCallback(attrName,oldValue,newValue) {
        ...
        if(attrName=="uid"){
            this.$wrapper.uid=newValue;
            this.loadVideoByUserID();    
            
        }
    }
    async loadVideoByUserID(){
        this.$wrapper.innerHTML="";
        
        let token=localStorage.getItem('token');
        let user=await firebase.firestore().collection('User').where("token","==",token).get();

        let _userID=user.docs[0].id;
        let datas=await firebase.firestore().collection('Video').where("userID","==",_userID).get();
