...
<div id="mySidenav" class="sidenav">
        ...
        <div>
            <a class="content" href="#/home" id="upload">Upload Video</a>
            <a class="content sign-out" href="#" id="login_button">Log out</a>
        </div>    
    </div>
    <div id="main">
        ...
        <div class="container mt-3" id="change-form">
            <div class="d-flex justify-content-start" id="div-video-wrapper">
                <div class="d-inlineblock">
                    <div class="video-wrapper d-flex justify-content-center w-100">
                        <iframe id="videosrc" autoplay=1 width="800" height="450" src="https://www.youtube.com/embed/GmrN4_Ealjk" title="YouTube video player" frameborder="0" allow="accelerometer;autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
                    </div>
                <div class="d-flex w-100 mt-3">
                    <h6 id="videoname" style="font-weight: bold;">hehe</h6> 
                </div>
            </div>
            <div id="app2" class="p-5"></div>
        </div>
    </div>  
</div>  
`;

export default class ViewVideo extends HTMLElement {
    ...
    constructor(){
        ...
        this.$app2 =this.shadowRoot.getElementById('app2');

        this.$upload = this.shadowRoot.getElementById('upload');
        this.$changeForm = this.shadowRoot.getElementById('change-form');

        this.$profile = this.shadowRoot.getElementById('profile');
        this.$loadType=this.shadowRoot.getElementById('load-type');
    }
    ...
    openNav() {
        ...
        this.shadowRoot.getElementById("main").style.marginLeft = "250px";
    }
    closeNav() {
        this.shadowRoot.getElementById("mySidenav").style.width = "0";
        this.shadowRoot.getElementById("main").style.marginLeft= "0";
    } 
    async changeProperties(){
        let token=localStorage.getItem('token')           
        let response = await firebase.firestore().collection('User').where("token","==",token).get();
        if(response.empty) {
            response = await firebase.firestore().collection('User').where("token","==",token).get();
            if (response.empty==false) {
                console.log(response.docs[0]);
                let _name=response.docs[0].data().name;
                this.$profile.innerHTML=_name;
            }
        }else{
        let _name=response.docs[0].data().name;
        this.$profile.innerHTML=_name;}

        response = await firebase.firestore().collection('Type').get();
        for(let re of response.docs){
            let $a=document.createElement('a');
            $a.innerHTML=re.data().name;
            $a.setAttribute("href","#/home");
            $a.setAttribute("id",re.data().name);
            this.$loadType.appendChild($a);
        }

        this.set_items();
    }

    loadVideoByType(_where){
        let $body=document.getElementById("body");
        $body.innerHTML='<div id="app"></div>';
        let $app=document.getElementById("app");
        let $allVideo=document.createElement("all-video");
        
        $app.appendChild($allVideo);
        $allVideo.loadVideoByType(_where);
    }

    set_items(){
        this.shadowRoot.getElementById("All").onclick=()=>{
            let $body=document.getElementById("body");
            $body.innerHTML='<div id="app"></div>';
            let $app=document.getElementById("app");
            $app.innerHTML='<all-video></all-video>';
        }
        this.shadowRoot.getElementById("Music").onclick=()=>{
            this.loadVideoByType("Music");
        }
        this.shadowRoot.getElementById("Game").onclick=()=>{
            this.loadVideoByType("Game");
        }
        this.shadowRoot.getElementById("Cartoon").onclick=()=>{
            this.loadVideoByType("Cartoon");
        }
        this.shadowRoot.getElementById("Sport").onclick=()=>{
            this.loadVideoByType("Sport");
        }
        this.shadowRoot.getElementById("News").onclick=()=>{
            this.loadVideoByType("News");
        }
    }
    ...
    connectedCallback(){
        ...
        let $videoForm=document.createElement('video-form-right');
        this.$app2.appendChild($videoForm);

        let $login_button = this.shadowRoot.getElementById('login_button');
        ...
        if (user==""||user==null) 
        {
            ...
            this.shadowRoot.getElementById("show-pf").style.display="none";
        }
        else{ 
            $login_button.innerHTML='Logout';
            this.$upload.style.display="block";
            this.shadowRoot.getElementById("show-pf").style.display="block";
        } 
        ...
        this.$upload.onclick=()=>{
            this.$changeForm.innerHTML="";
            let $videoForm=document.createElement('upload-form');
            this.$changeForm.appendChild($videoForm);
        }
        
        this.$profile.onclick=()=>{
            this.$changeForm.innerHTML="";
            let profile_form=document.createElement('profile-form');
            this.$changeForm.appendChild(profile_form);
        }
...
window.customElements.define("view-video-form",ViewVideo);